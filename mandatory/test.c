...
/*   Updated: 2024/09/12 04:23:32 by mabdessm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*int main()
{
	...
	// O_CREAT to create the file if it doesnt exist
	// 0644 is for the permissions if the file is to be created

	dup2(fd, STDOUT_FILENO);

	// STDOUT_FILENO is the standard output AKA 1
	// here dup2() redirects the standard output which would normally go to the
	// terminal to the file represented by the file descriptor fd, after this
	// any output to stdout will ne written to the file instead
...
/*int main()
{
	...
	// The reason the printf() line does not get executed is due to how the
	// execve() system call works. When execve() is called, it replaces the
	// current process image with a new program (in this case, the ls command).
	// After a successful execve(), the new program starts executing, and the
	// old program is no longer running.
	// Once execve() is executed successfully, control is passed to the new
	// program (ls -l), and the original code (including the printf("This line
	// will not be executed.\n");) is effectively discarded.
	// The process that was running the main() function is now running the ls
	// -l command. Therefore, anything after the execve() call (like the printf
	// statement) will not execute because the original program is no longer in
	// memory.
	// However, if execve() fails for some reason (e.g., the file doesn't exist
	// or isn't executable), it returns -1, and in that case, the printf() line
	// would execute.
...
/*int main()
{
	...
	// When fork() is called, it creates a new child process, which is an exact
	// copy of the parent process.
	// Both the parent and child processes continue executing from the point
	// where fork() was called, but their execution order depends on the
	// operating system's scheduler.
	// The parent process may run first, the child process may run first,
	// or they might even alternate back and forth. You cannot predict which
	// will execute first without explicit synchronization (like using wait()
	// or other mechanisms).
	// When a process is forked, the child will be at the same point in the
	// code as the parent: the child does not restart all the instructions from
	// the beginning! if there is a printf before the fork for example, it will
	// not be executed every time the process is forked
...
/*int main()
{
	...
	// when a process is forked, if the parent process changes the value of a
	// variable after creating its child process, the child will not see the
	// change if it checks the same variable hence why we need to use pipe()

	if (pipe(fd) == -1) //creates a pipe and initilizes fd[0] for reading and
						// fd[1] for writing
	{
		perror("pipe");
		exit(EXIT_FAILURE);
	...
	else //parent process
	{
		...
		// The parent and child processes use the pipe to communicate.
		// The child process writes a message to the pipe.
		// The parent process reads the message from the pipe and prints it.
		// This example demonstrates a simple form of inter-process communication
		// (IPC) using pipes.

		// If the child process writes data to the pipe before the parent
		// process reads from it, the parent process will successfully read the
		// message from the pipe and print it. If the child has not yet written
		// when the parent tries to read, the buffer might not contain the
		// expected data, but under normal circumstances with proper
		// synchronization, this issue should not occur.

		// The pipe(fd) function achieves the following:
		// Creates a communication channel between the parent and child
		// processes.
		// Allows the child process to send data to the parent process.
		// Enables the parent process to receive and use the data sent by the 
		// child.
		// This mechanism is crucial for IPC, allowing processes to exchange
